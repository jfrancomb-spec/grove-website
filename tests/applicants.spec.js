...
test("family dashboard job cards include an applicants button", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "View Applications" })).toHaveAttribute("href", "./applicants.html?job_id=job-1");
});
...
test("applicants page only shows conversations for the selected job", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "View Job: After-school childcare" })).toBeVisible();
